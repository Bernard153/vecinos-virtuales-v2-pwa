...
VV.marketplace = {
    ...
    loadShopping() {
        ...
        // Disparo de la IA de Cloudflare
...
}; // <--- IMPORTANTE: Este punto y coma cierra el objeto VV.marketplace
...
// ========== FUNCIONES GLOBALES (AFUERA DEL OBJETO) ==========
    ...
    try {
        ...
        const data = await res.json();
        const el = document.getElementById(`ai-promo-${id}`);
        if (el) el.innerText = "💡 Tip IA: " + (data.oferta || data.response || "¡Aprovecha!");
    } catch (e) {
        console.log("IA en espera...");
    }
}
...
async function mostrarGaleriaVendedor(idVendedor) {
    ...
    // Aquí puedes disparar un modal o scroll a la galería
        ...
        // Cargar precios colaborativos
        setTimeout(() => {
            if (typeof VV.marketplace.loadCollabPrices === 'function') {
                VV.marketplace.loadCollabPrices(neighborhoodProducts);
            }
        }, 100);
    },
    ...
    filterProducts() {
        ...
        container.innerHTML = filtered.map(p => `
            <div class="product-card">
                ...
                <!-- PRECIOS COLABORATIVOS -->
                <div id="collab-prices-filtered-${p.id}" class="collab-prices-container" style="display: none; padding: 0.75rem; background: var(--gray-50); border: 1px dashed var(--gray-300); border-radius: 8px; margin-top: 0.5rem;">
                </div>
                <div style="text-align: right; margin-top: 0.25rem; margin-bottom: 0.5rem;">
                    <button class="btn-text" onclick="VV.marketplace.showCollabPriceForm('${p.id}', '${p.product.replace(/'/g, "\\'")}')" style="background: none; border: none; color: var(--primary-purple); font-size: 0.8rem; cursor: pointer; text-decoration: underline;">
                        <i class="fas fa-search-dollar"></i> ¿Lo viste más barato?
        ...
        setTimeout(() => {
            if (typeof VV.marketplace.loadCollabPrices === 'function') {
                VV.marketplace.loadCollabPrices(filtered);
            }
        }, 100);
    },
    ...
    showCollabPriceForm(productId, productName) {
        let overlay = document.getElementById('collab-price-overlay');
        if (!overlay) {
            overlay = document.createElement('div');
            overlay.id = 'collab-price-overlay';
            overlay.className = 'modal-overlay';
            document.body.appendChild(overlay);
        }
        ...
        overlay.innerHTML = `
            <div class="modal-form" style="max-width: 400px;">
                <h3><i class="fas fa-search-dollar"></i> Precio Comunitario</h3>
                <p style="color: var(--gray-600); margin-bottom: 1rem; font-size: 0.9rem;">
                    ¿Viste este producto ("${productName}") más barato en otro lado? 
                    Compártelo con los vecinos para ayudarnos a ahorrar.
                </p>
                <form id="collab-price-form">
                    <div class="form-group">
                        <label>Precio que encontraste *</label>
                        <input type="number" id="collab-price-value" min="0" step="0.01" required placeholder="Ej: 1500">
                    </div>
                    <div class="form-group">
                        <label>¿En qué comercio/lugar? *</label>
                        <input type="text" id="collab-source" required placeholder="Ej: Supermercado del centro, Verdulería de la esquina">
                    </div>
                    <div class="form-actions">
                        <button type="button" class="btn-cancel" onclick="document.getElementById('collab-price-overlay').classList.remove('active')">Cancelar</button>
                        <button type="submit" class="btn-save">
        ...
        document.getElementById('collab-price-form').onsubmit = async (e) => {
            e.preventDefault();
            const price = parseFloat(document.getElementById('collab-price-value').value);
            const source = document.getElementById('collab-source').value.trim();

            try {
                const { error } = await supabase
                    .from('collaborative_prices')
                    .insert({
                ...
                if (error) {
                    if (error.code === '42P01') {
                        throw new Error('La tabla collaborative_prices no existe en Supabase. El administrador debe crearla primero.');
                    }
                    throw error;
                }

                overlay.classList.remove('active');
                VV.utils.showSuccess('¡Gracias por tu aporte!');

                // Aplicar el cambio de inmediato en la UI sin recargar todo para la UX
                const containers = document.querySelectorAll(`[id^="collab-prices-"][id$="${productId}"]`);
                const html = `
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        <span style="font-size: 0.75rem; color: var(--gray-600); font-weight: 600; text-transform: uppercase;">Mejor precio comunidad:</span>
                        <div style="color: var(--warning-orange); font-weight: 700;">$${price} <span style="font-size: 0.75rem; font-weight: normal; color: var(--gray-600);">en ${source}</span></div>
                    </div>
                </div>
            `;
    ...
    async loadCollabPrices(products) {
        ...
        try {
            ...
            data.forEach(item => {
                if (!pricesByProduct[item.product_id]) pricesByProduct[item.product_id] = [];
                pricesByProduct[item.product_id].push(item);
            });

            // Actualizar DOM
            products.forEach(p => {
                const prices = pricesByProduct[p.id];
                const containers = document.querySelectorAll(`[id^="collab-prices-"][id$="${p.id}"]`);

                if (prices && prices.length > 0) {
                    // Ordenar por precio más bajo
                    prices.sort((a, b) => a.found_price - b.found_price);
                    const best = prices[0];
                    ...
                    const html = `
                        <div style="display: flex; align-items: center; justify-content: space-between;">
                            <div>
                                <span style="font-size: 0.75rem; color: var(--gray-600); font-weight: 600; text-transform: uppercase;">Mejor precio comunidad:</span>
                                <div style="color: var(--warning-orange); font-weight: 700;">$${best.found_price} <span style="font-size: 0.75rem; font-weight: normal; color: var(--gray-600);">en ${best.source}</span></div>
                            </div>
                            ${prices.length > 1 ? `<div style="font-size: 0.7rem; color: var(--gray-500);">${prices.length} aportes</div>` : ''}
                        </div>
                    `;
                    containers.forEach(c => {
                        c.innerHTML = html;
            ...
            console.warn('No se pudieron cargar precios colaborativos:', e);
        }
    },
